...
class ListView(View):
    ...
    def get(self, request, category_id, page_num):
        ...
        # sku_qs = SKU.objects.filter(category_id=category_id, is_launched=True).order_by(sort_filed)
        # page = total_count / 5 + (1 if total % 5 else 0)  # 计算总页数
        # sa = 5 * (page_num - 1)
        # sku_qs[sa  : (sa + (5 - 1))]  # 计算显得指定页角标
        paginator = Paginator(sku_qs, 5)  # 创建分页对象
        try:
...
class DetailVisitView(View):
    ...
    def post(self, request, category_id):
        ...
        # now_date = timezone.localdate()  # 获取当前日期
